...
'''
args_to_array = '''
Convert a set of input arguments in a class that can be handled by compiled functions.

:param a: input array.
:type a: numpy.ndarray
'''
array_to_backend = '''
Process an array to be sent to another backend. If the other backend is the
same then no operation is done. Otherwise the necessary conversion is done
to adapt the array.
...
:returns: parsed array, whose type depends on the new backend.
:rtype: numpy.ndarray or reikna.cluda.Array
'''
concatenate = '''
...
.. note:: Input array must be of type :class:`minkit.operations.types.cpu_float`.
'''
meshgrid = '''
