 ...
 *
 */
#ifndef CUDA // Using OpenCL (set by reikna)
...
void atomicAdd(volatile __global double *addr, double val) {
  ...
  do {
    ...
    current.u =
        atomic_cmpxchg((volatile __global long *)addr, expected.u, next.u);
  } while (current.u != expected.u);
}
#endif // CUDA

// Sum the number of entries inside a bin
KERNEL void sum_inside_bins(GLOBAL_MEM double *out, int len,
                            GLOBAL_MEM double *data, int ndim,
...
KERNEL void sum_inside_bins_with_values(GLOBAL_MEM double *out, int len,
                                        GLOBAL_MEM double *data, int ndim,
                                        GLOBAL_MEM int *gaps,
